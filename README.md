...
$$
P(X) = \sum_{j=1}^{K} P_f(X) \cdot P_{g_j}(X \mid f(X))
$$

If \(P(f(x_i) = y_i) \approx 1\), this simplifies to:
...
2. For feature \(i\), permute feature column 1000 times and compute score drop  
Approximation:

$$
\frac{1}{n!} \sum_{\sigma_i} \left( s(F(X), y) - s(F(\sigma_i(X)), y) \right)
$$
